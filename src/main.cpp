...
{
	std::vector<Agent*> swarmAgents, SANextGen, predatorAgents, PANextGen;
	Agent *swarmAgent = nullptr, *predatorAgent = nullptr, *bestSwarmAgent = nullptr, *bestPredatorAgent = nullptr;
	double swarmMaxFitness = 0.0, predatorMaxFitness = 0.0;
    std::string LODFileName = "", swarmGenomeFileName = "", predatorGenomeFileName = "", inputGenomeFileName = "";
    std::string swarmDotFileName = "", predatorDotFileName = "", logicTableFileName = "";
	...
	game = new Game;
	swarmAgent = new Agent;
    predatorAgent = new Agent;
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            ++i;
            std::stringstream lodfn;
            lodfn << argv[i];
            LODFileName = lodfn.str();
            
            ++i;
            std::stringstream sgfn;
            sgfn << argv[i];
            swarmGenomeFileName = sgfn.str();
            
            ++i;
            std::stringstream pgfn;
            pgfn << argv[i];
        ...
        else if (strcmp(argv[i], "-g") == 0 && (i + 1) < argc)
        {
            ...
            {
                std::cerr << "minimum number of generations permitted is 3." << std::endl;
                exit(0);
            }
        }
        ...
        else if (strcmp(argv[i], "-t") == 0 && (i + 1) < argc)
        {
            ...
            {
                std::cerr << "minimum brain tracking frequency is 1." << std::endl;
                exit(0);
            }
        }
        ...
        else if (strcmp(argv[i], "-v") == 0 && (i + 1) < argc)
        {
            ...
            {
                std::cerr << "minimum video creation frequency is 1." << std::endl;
                exit(0);
            }
        }
    ...
    if (display_only)
    {
        std::string bestString = findBestRun(swarmAgent, predatorAgent);
        bestString.append("X");
    ...
    if (display_directory)
    {
        ...
        std::map< int, std::vector<std::string> > fileNameMap;
        ...
        if (dir != nullptr)
        {
            ...
            {
                std::string dirFile = std::string(ent->d_name);
                
                if (dirFile.find(".genome") != std::string::npos)
                {
                    ...
                    if (dirFile.find("swarm") != std::string::npos)
                    {
                        fileNameMap[runNumber][0] = argv[displayDirectoryArgvIndex] + dirFile;
                    }
                    else if (dirFile.find("predator") != std::string::npos)
                    {
        ...
        {
            std::cerr << "invalid directory: " << argv[displayDirectoryArgvIndex] << std::endl;
            exit(0);
        }
        
        // display every set of swarm/predator files
        for (std::map< int, std::vector<std::string> >::iterator it = fileNameMap.begin(), end = fileNameMap.end(); it != end; )
        {
            if (it->second[0] != "" && it->second[1] != "")
            {
                std::cout << "building video for run " << it->first << std::endl;
                ...
                std::cout << "displaying video for run " << it->first << std::endl;
            ...
            {
                std::cerr << "unmatched file: " << it->second[0] << " " << it->second[1] << std::endl;
            }
        }
    ...
    {
		swarmAgents[i] = new Agent;
		swarmAgents[i]->inherit(swarmAgent, 0.01, 1);
        
        predatorAgents[i] = new Agent;
	...
	for (int update = 1; update <= totalGenerations; ++update)
    {
		...
		for(int i = 0; i < populationSize; ++i)
        {
            game->executeGame(swarmAgents[i], predatorAgents[i], nullptr, false, safetyDist, predatorVisionAngle, killDelay, confusionMultiplier);
            
            // store the swarm agent's corresponding predator agent
            swarmAgents[i]->predator = new Agent;
            ...
            swarmAgents[i]->predator->ancestor = nullptr;
		...
		std::cout << "generation " << update << ": swarm [" << (int)swarmAvgFitness << " : " << (int)swarmMaxFitness << "] :: predator [" << (int)predatorAvgFitness << " : " << (int)predatorMaxFitness << "]" << std::endl;
        ...
        if (make_interval_video)
        {
            ...
            {
                std::string bestString = game->executeGame(bestSwarmAgent, bestPredatorAgent, nullptr, true, safetyDist, predatorVisionAngle, killDelay, confusionMultiplier);
        ...
        std::shuffle(SANextGen.begin(), SANextGen.end(), random);
        std::shuffle(PANextGen.begin(), PANextGen.end(), random);
    ...
    std::cout << "analyzing ancestor list" << std::endl;
    
    for (std::vector<Agent*>::iterator it = saveLOD.begin(); it != saveLOD.end(); ++it)
    {
        if ((*it)->predator == nullptr)
        {
            std::cout << "nullptr predator at " << (*it)->born << std::endl;
        }
        else
        {
...
std::string findBestRun(Agent *swarmAgent, Agent *predatorAgent)
{
    std::string reportString = "", bestString = "";
    double bestFitness = 0.0;
    
    for (int rep = 0; rep < 100; ++rep)
    {
        reportString = game->executeGame(swarmAgent, predatorAgent, nullptr, true, safetyDist, predatorVisionAngle, killDelay, confusionMultiplier);
...
void doBroadcast(std::string data)
{
    if ( (conn_s = accept(list_s, nullptr, nullptr) ) < 0 )
    {
