...
std::string Game::executeGame(Agent *swarmAgent, Agent *predatorAgent,
                              FILE *data_file, bool report, double safetyDist,
                              double predatorVisionAngle, int killDelay,
  ...
  std::vector<double> bbSizes;
  std::vector<double> shortestDists;
  std::vector<double> swarmDensityCounts;
  std::vector<int> predatorAngle, preyAngle;
  ...
  for (int i = 0; i < swarmSize; ++i) {
    ...
    do {
      ...
      for (int j = 0; j < i; ++j) {
        if (calcDistanceSquared(preyX[i], preyY[i], preyX[j], preyY[j]) <
            safetyDist) {
          goodPos = false;
          break;
        }
  ...
  recalcPredAndPreyDistTable(preyX, preyY, preyDead, predX, predY, predDists,
                             preyDists);
  ...
  for (int step = 0; step < totalStepsInSimulation; ++step) {
    ...
    if (report) {
      ...
      for (int i = 0; i < swarmSize; ++i) {
        if (!preyDead[i]) {
          ...
          sprintf(text, "%f,%f,%f,%d,%d,%d=", preyX[i], preyY[i], preyA[i], 255,
                  255, 255);
    ...
    if (data_file != nullptr) {
      ...
      // area = L x W
      //                L = dist (rbX, rbY) to (rbX, luY); W = dist (luX, luY)
      //                to (rbX, luY)
      bbSizes.push_back(sqrt(calcDistanceSquared(rbX, rbY, rbX, luY)) *
                        sqrt(calcDistanceSquared(luX, luY, rbX, luY)));
    ...
    for (int i = 0; i < swarmSize; ++i) {
      if (!preyDead[i]) {
        ...
        if (predDists[i] < predatorVisionRange) {
          ...
          // here we have to map the angle into the sensor, btw: angle in
          // degrees
          if (fabs(angle) < predatorVisionAngle) // predator has a limited
                                                 // vision field in front of it
          {
            predatorAgent
                ->states[(int)(angle / (predatorVisionAngle /
                                        ((double)predatorSensors / 2.0)) +
                               ((double)predatorSensors / 2.0))] = 1;
          }
        }
      }
    }
    ...
    //                                      node 31 node 30
    int action = ((predatorAgent->states[(maxNodes - 1)] & 1) << 1) +
                 (predatorAgent->states[(maxNodes - 2)] & 1);
    ...
    // recalculate the predator distances lookup table since the predator has
    // moved
      ...
      if (delay < 1) {
        ...
        for (int i = 0; !killed && i < swarmSize; ++i) {
          ...
          if (!preyDead[i] && (predDists[i] < killDist) &&
              fabs(calcAngle(predX, predY, predA, preyX[i], preyY[i])) <
                  predatorVisionAngle) {
            ++numAttacks;
            int nearbyCount = 0;

            for (int j = 0; j < swarmSize; ++j) {
              // other prey must be close to target prey and within predator's
              // retina
              if (!preyDead[j] && preyDists[i][j] < safetyDist &&
                  predDists[j] < predatorVisionRange &&
                  fabs(calcAngle(predX, predY, predA, preyX[j], preyY[j])) <
                      predatorVisionAngle) {
                ++nearbyCount;
              }
            }
      ...
      if (!preyDead[i]) {
        ...
        for (int j = 0; j < swarmSize; ++j) {
          ...
          if (i != j && !preyDead[j]) {
            ...
            if (d < preyVisionRange) {
              // ignore if agent i isn't even facing agent j (won't be within
              // retina)
              if (calcDistanceSquared(preyX[i] + cosLookup[(int)preyA[i]],
                                      preyY[i] + sinLookup[(int)preyA[i]],
                                      preyX[j], preyY[j]) < d) {
                double angle =
                    calcAngle(preyX[i], preyY[i], preyA[i], preyX[j], preyY[j]);

                // here we have to map the angle into the sensor, btw: angle in
                // degrees
                if (fabs(angle) < preyVisionAngle) // prey has a limited vision
                                                   // field infront of it
                {
                  swarmAgent
                      ->states[(int)(angle / (preyVisionAngle /
                                              ((double)preySensors / 2.0)) +
                                     ((double)preySensors / 2.0)) +
                               (i * maxNodes)] = 1;
                }
              }
        ...
        if (d < preyVisionRange) {
          ...
          if (fabs(angle) < preyVisionAngle) {
            swarmAgent->states[preySensors +
                               (int)(angle / (preyVisionAngle /
                                              ((double)preySensors / 2.0)) +
                                     ((double)preySensors / 2.0)) +
                               (i * maxNodes)] = 1;
          }
    ...
    // activate each swarm agent's brain, determine its action for this update,
    // and update its position and angle
    for (int i = 0; i < swarmSize; ++i) {
      if (!preyDead[i]) {
        //                                  node 31 node 30
        int action =
            ((swarmAgent->states[(maxNodes - 1) + (i * maxNodes)] & 1) << 1) +
            (swarmAgent->states[(maxNodes - 2) + (i * maxNodes)] & 1);
    ...
    // recalculate both the predator and prey distances lookup tables since the
    // entire swarm has moved
    recalcPredAndPreyDistTable(preyX, preyY, preyDead, predX, predY, predDists,
                               preyDists);
  ...
  if (data_file != nullptr) {
    fprintf(
        ...
        average(shortestDists), // average of avg. shortest distance to other
                                // swarm agent
        average(swarmDensityCounts), // average # of agents within 20 units of
                                     // each other
        neuronsConnectedToPreyRetina(
            swarmAgent), // # neurons connected to prey part of retina (prey)
        neuronsConnectedToPredatorRetina(
            swarmAgent), // # neurons connected to predator part of retina
                         // (prey)
        neuronsConnectedToPreyRetina(
            predatorAgent), // # neurons connected to prey part of retina
                            // (predator)
        mutualInformation(predatorAngle,
                          preyAngle), // mutual Information between prey flight
                                      // angle and predator flight angle
        numAttacks);
  }
...
double Game::calcDistanceSquared(double fromX, double fromY, double toX,
                                 double toY) {
...
double Game::calcAngle(double fromX, double fromY, double fromAngle, double toX,
                       double toY) {
...
void Game::recalcPredDistTable(double preyX[], double preyY[], bool preyDead[],
  ...
  for (int i = 0; i < swarmSize; ++i) {
    if (!preyDead[i]) {
      ...
      for (int j = i + 1; j < swarmSize; ++j) {
        if (!preyDead[j]) {
          preyDists[i][j] =
              calcDistanceSquared(preyX[i], preyY[i], preyX[j], preyY[j]);
...
void Game::applyBoundary(double &positionVal) {
...
double Game::mutualInformation(std::vector<int> A, std::vector<int> B) {
  std::set<int> nrA, nrB;
  std::set<int>::iterator aI, bI;
  std::map<int, std::map<int, double>> pXY;
...
double Game::entropy(std::vector<int> list) {
  std::map<int, double> p;
  std::map<int, double>::iterator pI;
  int i;
  double H = 0.0;
...
double Game::ei(std::vector<int> A, std::vector<int> B, int theMask) {
  std::set<int> nrA, nrB;
  std::set<int>::iterator aI, bI;
  std::map<int, std::map<int, double>> pXY;
  std::map<int, double> pX, pY;
...
double Game::computeAtomicPhi(std::vector<int> A, int states) {
  int i;
  double P, EIsystem;
  ...
  //	cout<<-EIsystem+P<<" "<<EIsystem<<" "<<P<<" "<<T0.size()<<"
  //"<<T1.size()<<endl;
  return -EIsystem + P;
}

double Game::computeR(std::vector<std::vector<int>> table, int howFarBack) {
...
double Game::computeOldR(std::vector<std::vector<int>> table) {
  double Ia, Ib;
...
double Game::predictiveI(std::vector<int> A) {
  std::vector<int> S, I;
  S.clear();
  I.clear();
  for (int i : A) {
...
double Game::nonPredictiveI(std::vector<int> A) {
  std::vector<int> S, I;
  S.clear();
  I.clear();
  for (int i : A) {
...
double Game::predictNextInput(std::vector<int> A) {
  std::vector<int> S, I;
  S.clear();
  I.clear();
  for (int i : A) {
...
void Game::loadExperiment(char *filename) {
...
int Game::neuronsConnectedToPredatorRetina(Agent *agent) {
  ...
  for (i = 0; i < A->hmmus.size(); i++)
    for (j = 0; j < A->hmmus[i]->ins.size(); j++)
      if ((A->hmmus[i]->ins[j] < (preySensors * 2)) &&
          (A->hmmus[i]->ins[j] >= preySensors))
        c++;
  delete A;
  return c;
}
...
void Octuplet::loadOctuplet(FILE *f) {
  int i, IN;
  data.clear();
  data.resize(8);
  for (i = 0; i < 8; i++) {
...
void Experiment::loadExperiment(char *filename) {
