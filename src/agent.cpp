...
void Agent::saveToDot(const char *filename, bool predator) {
...
void Agent::saveLogicTable(const char *filename) {
  ...
  for (i = 0; i < (int)pow(2.0, 13.0); i++) {
    ...
    for (int repeat = 1; repeat < NUM_REPEATS; ++repeat) {
      ...
      if (repeat == (NUM_REPEATS - 1)) {
        ...
        fprintf(f, ",%i,%i\n", mostCommonOutput->first[0],
                mostCommonOutput->first[1]);
      }
    }
  }
